...
struct TimeoutsThread {
    ...
    //processed
    loopback_channel: ChannelSyncTx<Timeout>,
...
impl Timeouts {
    ...
    pub fn new(iteration_delay: u64, loopback_channel: ChannelSyncTx<Timeout>) -> Self {
        let tx = TimeoutsThread::new(iteration_delay, loopback_channel);
...
impl TimeoutsThread {
    fn new(iteration_delay: u64, loopback_channel: ChannelSyncTx<Timeout>) -> ChannelSyncTx<TimeoutMessage> {
    ...
    fn run(mut self) {
        ...
        loop {
            ...
            if !to_time_out.is_empty() {
                ...
                if let Err(_) = self.loopback_channel.send(to_time_out) {
