...
use futures::io::{BufReader, BufWriter, ReadHalf};
...
pub struct AsyncSendSocket {
    #[cfg(feature = "socket_tokio_tcp")]
    inner: tokio_tcp::WriteSocket,

    #[cfg(feature = "socket_async_std_tcp")]
    inner: async_std_tcp::WriteSocket,
}

pub struct AsyncRecvSocket {
    #[cfg(feature = "socket_tokio_tcp")]
    inner: tokio_tcp::ReadSocket,

    #[cfg(feature = "socket_async_std_tcp")]
    inner: async_std_tcp::ReadSocket,
}
...
pub enum SyncSendSocket {
    Plain(std_tcp::WriteSocket),
    Tls(std_tcp::WriteSocket)
}

pub struct SyncRecvSocket {
    inner: std_tcp::ReadSocket
}
...
pub enum SecureSocketSendAsync {
    ...
    Tls(BufWriter<TlsStreamCli<AsyncSocket>>),
}
...
impl SecureSocketSendAsync {
    pub fn new_plain(async_socket: AsyncSocket) -> Self {
        SecureSocketSendAsync::Plain(BufWriter::new(async_socket))
    }
    
    pub fn new_tls(tls_stream_client: TlsStreamCli<AsyncSocket>) -> Self  {
        SecureSocketSendAsync::Tls(BufWriter::new(tls_stream_client))
    }
}

impl SecureSocketRecvAsync {
    pub fn new_plain(async_socket: AsyncSocket) -> Self {
        SecureSocketRecvAsync::Plain(BufReader::new(async_socket))
    }
...
impl SecureSocketRecvSync {
    ...
    pub fn new_plain(sync_socket: SyncSocket) -> Self {
        SecureSocketRecvSync::Plain(std::io::BufReader::new(sync_socket))
    }
...
impl SecureSocketSendSync {
    ...
    pub fn new_plain(sync_socket: SyncSocket) -> Self {
        SecureSocketSendSync::Plain(std::io::BufWriter::new(sync_socket))
    }
...
impl Write for SyncSendSocket {
    fn write(&mut self, buf: &[u8]) -> io::Result<usize> {
        let inner = match self {
            SyncSendSocket::Plain(inner) => {inner}
            SyncSendSocket::Tls(inner) => {inner}
        };
