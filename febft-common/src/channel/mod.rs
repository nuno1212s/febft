...
// Since they are so simple and one shot is so good, it's just a reference to the actual types,
// just to maintain the design pattern
pub type OneShotChannelTx<T> = oneshot_::OneShotTx<T>;

pub type OneShotChannelRx<T> = oneshot_::OneShotRx<T>;

#[inline]
pub fn oneshot_channel<T>() -> (OneShotChannelTx<T>, OneShotChannelRx<T>) {
    oneshot_::channel()
}
