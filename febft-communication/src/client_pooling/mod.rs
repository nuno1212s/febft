...
/// actually interacting with the TcpStreams, serializing and verifying the messages.
/// This only handles already serialized messages.
        ...
        match self {
            Self::PoolConnection { queue, client_id, .. } => {
                ...
                match &mut *sender_guard {
                    None => {
                        error!("Failed to push request into client {:?} as he was already disconnected", client_id);
