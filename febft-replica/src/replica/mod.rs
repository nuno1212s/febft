...
use febft_communication::message::{Header, NetworkMessage, NetworkMessageContent, StoredMessage};
use febft_consensus::consensus::{Consensus, ConsensusGuard, ConsensusPollStatus, ConsensusStatus};
...
use febft_common::channel::{ChannelSyncRx, new_bounded_sync};
use febft_communication::{Node, NodeConfig, NodeId};
use febft_consensus::messages::{ConsensusMessage, CstMessage, ObserveEventKind, PBFTProtocolMessage, ViewChangeMessage};
use febft_messages::messages::{ForwardedRequestsMessage, RequestMessage, SystemMessage};
#[cfg(feature = "serialize_serde")]
use serde::{Deserialize, Serialize};
use crate::executor::{AppState, Executor, ReplicaReplier};
...
pub struct Replica<S: Service + 'static> {
    ...
    checkpoint_delivery_rx: ChannelSyncRx<AppState<S>>,
    timeout_delivery_rx: ChannelSyncRx<Timeout>,
        ...
        Reply<S>: Send + 'static,
{
    ...
    pub async fn bootstrap<T>(cfg: ReplicaConfig<S, T>) -> Result<Self> where T: PersistentLogModeTrait {
        ...
        let (timeout_tx, timeout_rx) = new_bounded_sync(1024);

        debug!("Initializing timeouts");
        // start timeouts handler
        let timeouts = Timeouts::new(500, timeout_tx);
        ...
        let mut replica = Replica {
            ...
            pending_request_log,
            decided_log,
            timeout_delivery_rx: timeout_rx,
        };
        ...
        for message in rogue {
            ...
            match message.into() {
                SystemMessage::OrderedRequest(req) => {
                    replica.request_received(header, req);
                }
                SystemMessage::UnorderedRequest(req) => {

                }
                SystemMessage::OrderedReply(_) | SystemMessage::UnorderedReply(_) => {}
                SystemMessage::ForwardedRequests(forwarded) => {}
    ...
    fn update_sync_phase(&mut self) -> Result<bool> {
        ...
        match message.into() {
            ...
            _ => {
                warn!("Rogue system message detected in replicas");
            }
        }
