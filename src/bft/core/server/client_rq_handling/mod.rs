...
use crate::bft::communication::message::{Message, RequestMessage, StoredMessage, SystemMessage};
...
pub struct RqProcessor<S: Service, O: SharedData> {
    batch_channel: (ChannelTx<Vec<StoredMessage<RequestMessage<O::Request>>>>,
                    ChannelRx<Vec<StoredMessage<RequestMessage<O::Request>>>>),
    replica_ref: Arc<Replica<S>>,
...
impl<S: Service, O: SharedData> RqProcessor<S, O> {
    pub fn new(node: Arc<Replica<S>>) -> Arc<Self> {
    ...
    pub fn start(self: Arc<Self>) -> JoinHandle<()> {
        std::thread::spawn(move || {
            loop {
                ...
                let messages = rt::block_on(self.replica_ref.receive_from_clients()).unwrap();

                //We only want to produce batches to the channel if we are the leader, as
                //Only the leader will propose things
                let is_leader = self.replica_ref.synchronizer.view().leader() == self.replica_ref.id();
                        ...
                        Message::System(header, sysmsg) => {
                            match &sysmsg {
                                SystemMessage::Request(req) => {
                                    ...
                                    self.replica_ref.request_received(header, sysmsg);
                                }
    ...
    pub fn rx_channel(&self) -> ChannelRx<Vec<StoredMessage<RequestMessage<O>>>> {
