use std::{collections::VecDeque, sync::{Mutex, Arc, atomic::Ordering}, ops::{DerefMut, Deref}};
...
use crate::bft::{communication::{NodeId, message::{StoredSerializedSystemMessage, StoredMessage, RequestMessage, ConsensusMessageKind, SystemMessage, ConsensusMessage, Header, ObserveEventKind, SerializedMessage, WireMessage}, Node, serialize::DigestData}, executable::{Request, Service, State, Reply}, crypto::hash::Digest, core::server::{observer::{ObserverHandle, MessageType}, ViewInfo}, ordering::{SeqNo, Orderable}, sync::{AbstractSynchronizer, replica_sync::Synchronizer}, timeouts::TimeoutsHandle, cst::RecoveryState, threadpool};

use crate::bft::ordering::tbo_pop_message;

use super::{ProtoPhase, TboQueue, ConsensusGuard, log::MemLog, ConsensusPollStatus, ConsensusStatus, AbstractConsensus};
...
impl<S: Service> AbstractConsensus<S> for Consensus<S> {
    ...
    Reply<S>: Send + 'static,
{
    ...
    pub fn new(
        ...
        Self {
            ...
            current: std::iter::repeat_with(|| Digest::from_bytes(&[0; Digest::LENGTH][..]))
                ...
                .take(batch_size)
                .collect(),
            consensus_guard: ConsensusGuard::new(initial_seq_no, view),
            observer_handle,
        }
    }

    pub fn consensus_guard(&self) -> &ConsensusGuard {
    ...
    ) 
    {
        ...
        self.process_message(header, message, timeouts, synchronizer, log, node);
        ...
        match self.phase {
            ...
            ProtoPhase::Preparing(i) => {
                ...
                self.phase = if i == curr_view.params().quorum() {
                    ...
                    if let Err(err) = self.observer_handle.tx().send(MessageType::Event(
                        ObserveEventKind::Commit(self.sequence_number()),
                    )) {
                        error!("{:?}", err);
                    }
            ...
            ProtoPhase::Committing(i) => {
                ...
                if i == synchronizer.view().params().quorum() {
                    ...
                    if let Err(_) = self.observer_handle.tx().send(MessageType::Event(
                        ObserveEventKind::Consensus(self.sequence_number()),
                    )) {
                        warn!("Failed to notify observers of the consensus instance")
                    }
