...
use febft_communication::message::{Header, NetworkMessageContent, StoredMessage};
use febft_communication::Node;
use febft_execution::executable::{ExecutorHandle, Request};
use febft_execution::executable::Service;
use febft_execution::executable::UnorderedBatch;
...
impl<S: Service + 'static> Proposer<S> {
                ...
                loop {
                    ...
                    if let Some(messages) = opt_msgs {
                        ...
                        for message in messages {
                            let (header, sysmsg) = message.into_inner();

                            if let NetworkMessageContent::System(sys_msg) = sysmsg {
                                match sys_msg {
    ...
    fn propose(
        ...
        self.node_ref.broadcast_signed(NetworkMessageContent::System(message.into()), targets);
    }
