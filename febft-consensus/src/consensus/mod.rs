...
use febft_common::ordering::{Orderable, SeqNo, tbo_advance_message_queue, tbo_pop_message, tbo_queue_message};
...
use crate::msg_log::Info;
use crate::msg_log::pending_decision::PendingRequestLog;
use crate::observer::ObserverHandle;
use crate::sync::{AbstractSynchronizer, Synchronizer};
...
macro_rules! extract_msg {
    ...
    ($opt:block, $rsp:expr, $g:expr, $q:expr) => {
        if let Some(stored) = tbo_pop_message::<StoredMessage<ConsensusMessage<_>>>($q) {
            $opt
